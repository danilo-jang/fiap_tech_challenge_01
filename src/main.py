...
async def get_producao(ano: int):
    try:
        url = f"{source_url}/index.php?opcao=opt_02"
        if ano:
...
async def get_processamento(sub_opcao: str, ano: int):
    try:
        ...
        url = f"{source_url}/index.php?opcao=opt_03&subopcao=subopt_{sub_opcao}"
        if ano:
            url = url + f"&ano={str(ano)}" 
...
async def get_comercializacao(ano: int):
    try:
        url = f"{source_url}/index.php?opcao=opt_04"
        if ano:
            url = url + f"&ano={str(ano)}" 
...
async def get_importacao(ano: int, sub_opcao: str = ""):
    try:
        print(f"sub_opcao {sub_opcao}")
        url = f"{source_url}/index.php?opcao=opt_05"
        if sub_opcao:
...
async def get_exportacao(sub_opcao: str, ano: int):
    try:
        print(f"sub_opcao {sub_opcao}")
        url = f"{source_url}/index.php?opcao=opt_06&subopcao=subopt_{sub_opcao}"
        if ano:
            url = url + f"&ano={str(ano)}" 
